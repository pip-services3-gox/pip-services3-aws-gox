...
import (
	...
	awsconn "github.com/pip-services3-gox/pip-services3-aws-gox/connect"
...
func (c *CloudWatchLogger) Write(ctx context.Context, level clog.LevelType, correlationId string, ex error, message string) {
	if c.Level() < level {
		return
	}
	c.CachedLogger.Write(ctx, level, correlationId, ex, message)
}
...
func (c *CloudWatchLogger) Open(ctx context.Context, correlationId string) error {
	if c.IsOpen() {
		return nil
	}
...
//		- correlationId (optional) transaction id to trace execution through call chain.
//		- Returns       error or nil no errors occured.
func (c *CloudWatchLogger) Close(ctx context.Context, correlationId string) error {
...
func (c *CloudWatchLogger) Save(ctx context.Context, messages []clog.LogMessage) error {
	...
	go func() {
		...
		if putErr != nil {
			if c.logger != nil {
				c.logger.Error(ctx, "cloudwatch_logger", putErr, "putLogEvents error")
			}
		} else {
